...
function DoctorDetails() {
  ...
  useEffect(() => {
    axios
      .post("https://localhost:3000/admin/doctorDetails", { id })
      .then((res) => {
  ...
  const handleClick = async (userId) => {
    try {
      await axios.patch("https://localhost:3000/admin/doctorblockUnblock", {
        userId,
      });
      const res = await axios.post(
        "https://localhost:3000/admin/doctorDetails",
        { id }
      );
      setDoctorData(res?.data?.details);
    } catch (error) {
