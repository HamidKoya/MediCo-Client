...
function ChatBox({ chat, currentUser, setMessages, messages, socket }) {
    ...
    const getDoctorData = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:3000/chat/doctorData/${doctorId}`
        );
        setDoctorData(data);
      } catch (error) {
        console.log(error.message);
              ...
              {messages.map((message) => (
                <div ref={scroll} key={message._id}>
                  ...
                  {message.text.includes(
                    "https://healthgoody.vercel.app/doctor/video?roomID="
                  ) ? (
